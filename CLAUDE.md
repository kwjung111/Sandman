...
| `.github/workflows/build-push.yml` | CI/CD — main 푸시 시 이미지 빌드 & 레지스트리 푸시. 레지스트리 주소·프로젝트는 repo Variables, 자격증명은 Secrets (저장소에 하드코딩 금지) |
| `start.sh` | (Linux) Docker 컨테이너 실행 (`--env-file` 방식, 이미지는 `IMAGE` env로 지정 가능) |
| `Dockerfile` | Python 3.11-slim 기반 멀티 스테이지 빌드 지향 이미지 |

## 배포

- CI/CD: main 푸시 → GitHub Actions가 빌드·푸시 (태그: git short SHA + latest)
- 서버: `start.sh`가 `.env`의 `IMAGE`를 pull 후 컨테이너 교체
